...
| ------------------- | ---------------------------------------------------------------- | -------------------------------------------------------------------------------------------- |
| Credentials         | `scope=active\|trash`, trash side-effects, flat Trash UI (C0–C1) | 🟡 PARTIAL — naming `archivedAt` transitional; C1.8 tests, C2 polish backlog                 |
...
### Phase 4 — Finance — P1 🟡 IN PROGRESS
...
| 4.2 | Invoice / Order / Payment: void/cancel flows вместо delete          | 🟡 PARTIAL — Invoice cancel + draft delete guards; Order draft delete; Payment delete = ledger reversal (open period) |
| 4.3 | Expense: delete только `PLANNED`/draft; иначе cancel                | ✅ DONE                                                                                                               |
| 4.4 | Expense Plan: trash или deactivate? решить по связям/учету          | ✅ DONE — empty-only hard delete; deactivate = `autoGenerate: false` via update                                       |
| 4.5 | Web: убрать destructive delete для posted records                   | 🟡 PARTIAL — Expense UI delete/cancel split; Invoice/Order web delete unused                                          |
| 4.6 | Journal / audit trail: все void actions logged                      | ⬜ TODO — accrual lines stay; explicit void journal rows backlog                                                      |
...
| Order        | `PENDING_PAYMENT`, no invoices          | status `CLOSED` / no delete (backlog) |
