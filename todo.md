...
- [x] Содержимое ядер: сначала 25 черновиков (5 видов × 5 размеров). **2026-09-21 решение 1.18:**
      размер снят. Один вид — одно ядро. Комплекты — именованные помощники (Базовый / Расширенный /
      Полный). **2026-09-21:** сид покрывает 8 видов enum (магазин, сайт компании, лендинг, визитка,
      CRM, web app, ERP, SaaS). OTHER, маркетинг и `MOBILE_APP` не сеем. Ядро `mobile-app-code` не
      предлагается на экране норм. `pnpm seed:delivery-profiles`.
- [x] Каталог наполнен: 202 карточки, 18 категорий, юниты по ролям черновиками (2026-09-20).
      ...
      Масштаб: 1 юнит = 1000 AMD себестоимости. Сид 10 000 / ИИ 20 000 с 2026-09-23 — вся цена карточки, не ставка за unit.
- [x] Экран каталога: левая колонка категорий, поиск, блоки по категориям, мелкие карточки; тот же
      ...
      **2026-09-23:** каталог и нормы сведены в шапку **Core & Function**
      (`/my-company/delivery-norms`): табы Core / Functions / Role rates; связанные вещи в sheet
      карточки (как invoice). Старый `/my-company/function-catalog` редиректит на Functions.
      Браузерная проверка — в этом срезе.
...
- [x] Цена продажи (решение 1.12) целиком: версионность, API и редактор. Множитель и фикс отозваны
      2026-09-21 (`647d12b2e`). Неявный дефолт «пусто = 10 000» отозван 2026-09-21 (`2231f2ba6`).
      **2026-09-23:** число на карточке — вся цена функции, градации и ядра, не AMD за unit. Сид
      публикует 10 000 / ИИ 20 000 как эту сумму. Нет суммы — нет цены.
- [x] Ось `platform` (web / app / desktop) как отдельный признак сделки. На units не влияет:
      ...
      `MOBILE_APP` (2026-09-21, `15dc34958`). **2026-09-21 уточнение:** поле только у Code (WEB/APP/
      DESKTOP) и WP/Shopify (WEB); Marketing — `NULL`, поля нет. `MOBILE_APP` скрыт из нового выбора
      вида, enum для legacy остаётся. App Store-слот смотрит на platform APP. **2026-09-21 решение
      1.22:** умный список видов Code по платформе (сайты только WEB; APP/DESKTOP — shop/CRM/ERP/
      SaaS/WEB_APP). Новые значения enum в этом срезе не добавляли.
...
- [ ] Объём экземпляра (решение 1.24): коэффициент ×0.0…×2.0 на ядре и на каждой добавленной
      функции, причина вне стандарта, метка на карточке доски. Миграция dev не применена.
      Браузерная проверка не выполнена.

- [x] Типы продуктов: ревизия `ProductTypeEnum` под направления Code (29 видов, решение 1.23).
      `MOBILE_APP` / `SAAS` скрыты. **2026-09-21:** на `ep-nameless-term` залиты и опубликованы
      29 ядер, 218 функций (13 extra + категория `learning`), список `PRODUCT_TYPE`. Production
      не трогали. Юниты — сид-предложение, Owner правит сам. Hover на Deal — позже.
      Ревью 1.23 закрыто: норма функции пишется на градацию (`tierId`), legacy SAAS/MOBILE_APP
      нельзя менять друг на друга, Deal picker режет неактивные system-list коды, подписи видов
      через `forms.product.types`, сид `PRODUCT_TYPE` из shared, publish берёт newest draft
      на карточку+градацию.
      **2026-09-24 решение 1.25:** пять дополнительных WEB-видов (недвижимость, услуги, туризм,
      объявления, вакансии) добавлены в код и черновики ядер без units/цены. Миграция и сид на
      живой базе не выполнялись; публикация норм остаётся за Owner.
...
параметров (`2302d0392`). Размер из этого ключа снят решением 1.18. **2026-09-21 решение 1.21:**
основа, три режима дизайна и AI-reviewer сняты с выбора; enrollment сам биндит опубликованное ядро
по виду продукта.
...
waiver не делаем; переоткрытие без изменения scope; каталог наполняет исполнитель, владелец проверяет цифры.
Цена продажи (уточнение владельца 2026-09-23): общая сумма на версии карточки функции, градации и ядра;
units её не меняют. Прежнее «AMD за unit» отозвано.
