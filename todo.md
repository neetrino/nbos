...
- [ ] `packages/shared` — `EntityLifecycleScope = active|trash`, `LifecycleProfile` enum (A–G), action name constants
- [ ] `apps/api` — `buildScopeWhere(scope)`, `assertTrashable()`, `assertRestorable()`, link guards
- [ ] `apps/web` — `useListScope()` hook: scope + reset navigation on change (R1–R3)
- [ ] Документ `docs/NBOS/03-Business-Logic/XX-Entity-Lifecycle-Standard.md` — Framework + Profiles A–G
...
| #   | Задача                                                                           | Статус                          |
| --- | -------------------------------------------------------------------------------- | ------------------------------- | ------- |
| 0.1 | Canon doc: Framework + Profiles A–G + rules R1–R5 + Archive vs Trash terminology | ⬜ TODO                         |
| 0.2 | Shared types: `EntityLifecycleScope`, `LifecycleProfile`, API naming             | ⬜ TODO                         |
| 0.3 | Nest: `buildScopeWhere()`, trash/restore guards                                  | ⬜ TODO                         |
| 0.4 | Web: `useListScope` pattern (scope switch + navigation reset)                    | ⬜ TODO                         |
| 0.5 | Закрыть open questions O1–O5                                                     | ⬜ TODO                         |
| 0.6 | Миграция query params: `includeArchived` / `isArchived` → `scope=active          | trash` (Credentials в Phase C1) | ⬜ TODO |
...
| #    | Задача                                                                                                           | Статус  |
| ---- | ---------------------------------------------------------------------------------------------------------------- | ------- |
| C0.1 | При `vaultListScope === 'trash'` (transitional `archived`): force view `list` или `tiles` (disable folders mode) | ⬜ TODO |
| C0.2 | `setVaultListScope('trash')` → reset `activeFolderId`, `activeProjectId`                                         | ⬜ TODO |
| C0.3 | Trash: не вызывать `listFolders` / `listProjectShells`                                                           | ⬜ TODO |
| C0.4 | Web tests / manual: Trash не показывает active folder blocks                                                     | ⬜ TODO |
...
| #    | Задача                                                                                                                        | Статус  |
| ---- | ----------------------------------------------------------------------------------------------------------------------------- | ------- |
| C1.1 | Единый `vaultContext=active\|trash` на list endpoints (credentials, folders, project-shells)                                  | ⬜ TODO |
| C1.2 | `trashCredential` / transitional `archiveCredential` → `deleteMany` folder memberships в той же транзакции                    | ⬜ TODO |
| C1.3 | Folder `credentialCount` = memberships где credential not trashed (`trashedAt IS NULL`; transitional `archivedAt IS NULL`)    | ⬜ TODO |
| C1.4 | **Replace `archiveFolder`:** `deleteFolder` только если empty (0 memberships active creds, 0 child folders) → hard delete row | ⬜ TODO |
| C1.5 | Непустая папка → `409` / `BadRequest` с текстом «Move credentials to Trash or remove them first»                              | ⬜ TODO |
| C1.6 | Deprecate/remove `DELETE folders/:id` archive path; audit `credential_folder.deleted`                                         | ⬜ TODO |
| C1.7 | `listProjectShells` — только active credentials                                                                               | ⬜ TODO |
...
| #    | Задача                                                                    | Статус  |
| ---- | ------------------------------------------------------------------------- | ------- |
| C2.1 | Folder card: «Delete» вместо «Archive»; confirm только для пустой папки   | ⬜ TODO |
| C2.2 | Trash: filters (category, project, sort by trash date)                    | ⬜ TODO |
| C2.3 | Restore credential: toast «Returned to vault (unfiled)» — без auto-folder | ⬜ TODO |
...
| #   | Задача                                                                                                          | Статус  |
| --- | --------------------------------------------------------------------------------------------------------------- | ------- |
| 6.1 | Drive: схлопнуть Archive + Trash UI в один Trash view                                                           | ⬜ TODO |
| 6.2 | Drive API: `archiveFileAsset`/`restoreFileAsset` заменить или адаптировать под `moveToTrash`/`restoreFromTrash` | ⬜ TODO |
| 6.3 | Drive DB: решить миграцию `ARCHIVED`/`archivedAt` → `DELETED`/`deletedAt` или новый `TRASHED`/`trashedAt`       | ⬜ TODO |
