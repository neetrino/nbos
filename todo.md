...
Очередь сейчас: Mail BullMQ (обсуждение). Дальше по плану — WhatsApp Won-модалка. Очередь групп оставляем.
...
- [x] WhatsApp: крон-ремонт и batch-reconcile **удалены** (старые мигрированные продукты без группы не трогать). Очередь и воркер оставить. Пока автосоздание ещё вызывается у нового Product / Deal Won.
- [ ] **План — WhatsApp на Won (PRODUCT / OUTSOURCE):** перед успешным закрытием модалка, **без кнопки «пропустить»**. Два пути: (1) «Создать группу» — очередь как сейчас; если WAHA лежит и создание упало — **всё равно можно идти дальше**; когда WAHA живой — группа создаётся. (2) Вставить ID уже существующей группы и сохранить — тоже можно идти дальше. Автокод `ensure` не удалять; тихий вызов при Product/Won выключить, чтобы группу не создавало незаметно. MAINTENANCE / EXTENSION — не этот гейт (новую группу не плодить). Та же логика, если WhatsApp снова отвалится.
