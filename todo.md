...
Очередь сейчас: Mail runtime A+B+C на проде (`23c72cb4`). Почтовые cron 🟡 не включали. WhatsApp Won на проде. Очередь групп оставляем.
...
- [ ] Mail — осталось после выкладки (не новый срез A/B/C):
  ...
  - [ ] Включить почтовые cron **по одному**, не пакетом (реестр 18–20, default off):
    - [ ] `SCHEDULER_MAIL_OUTBOUND_RECONCILE_ENABLED` — застрявшие исходящие (QUEUED / stale SENDING).
    - [ ] `SCHEDULER_MAIL_GMAIL_WATCH_RENEW_ENABLED` — продление Gmail watch (только если есть Gmail-ящики).
    - [ ] `SCHEDULER_MAIL_SYNC_RECONCILE_ENABLED` — сетка inbox раз в 5 мин; без неё новые письма только пока живы Pub/Sub / IDLE / ручной Sync.
