...
- [x] **4. Query layer** — `work-space-queries.ts` (keys, fetch bundle, 404 → ensure)
...
- [x] **7. Follow-up** — `workSpaceId` on `GET /products/:id`; standalone `/work-spaces/[id]` on Query; invalidate/refetch on sprint bulk task refresh
