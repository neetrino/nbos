...
describe('delivery catalog seed data', () => {
  ...
  it('proposes positive units for every card', () => {
    const withoutUnits = DELIVERY_CATALOG_SEED_ITEMS.filter(
      (item) => totalSeedUnits(item.units) <= 0,
    ).map((item) => item.code);
  ...
  it('never proposes a negative or unknown role', () => {
    ...
    for (const item of DELIVERY_CATALOG_SEED_ITEMS) {
      for (const [role, units] of Object.entries(item.units)) {
