...
function validateRules() {
  ...
  for (const file of files) {
    ...
    // Template migrated onboarding/size into Skills. NBOS keeps 21/99 as project-local rules.
    if (/\.cursor\/skills/.test(source)) report(errors, file, 'Rule references a removed or migrated Skill path.');
  }
  return files.length;
}
