/* !!! This is code generated by Prisma. Do not edit directly. !!! */
/* eslint-disable */
// biome-ignore-all lint: generated file
// @ts-nocheck
/*
 * This is a barrel export file for all models and their related types.
 *
 * 🟢 You can import this file directly.
 */
export type * from './models/AuditLog';
export type * from './models/CalendarMeeting';
export type * from './models/PersonalCalendarEvent';
export type * from './models/ChecklistTemplate';
export type * from './models/ChecklistTemplateVersion';
export type * from './models/ChecklistInstance';
export type * from './models/DeliveryStageChecklistRule';
export type * from './models/ClientServiceRecord';
export type * from './models/Contact';
export type * from './models/Company';
export type * from './models/Credential';
export type * from './models/Domain';
export type * from './models/MarketingAccount';
export type * from './models/MarketingActivity';
export type * from './models/MarketingCrmWhereOption';
export type * from './models/Lead';
export type * from './models/Deal';
export type * from './models/DocumentSection';
export type * from './models/ExternalDocumentLink';
export type * from './models/Document';
export type * from './models/DocumentTag';
export type * from './models/DocumentTagOnDocument';
export type * from './models/DocumentAttachment';
export type * from './models/DocumentActivityEvent';
export type * from './models/FileAsset';
export type * from './models/DriveFolder';
export type * from './models/DriveFolderItem';
export type * from './models/FileVersion';
export type * from './models/FileLink';
export type * from './models/FileAuditEvent';
export type * from './models/FileUploadSession';
export type * from './models/Employee';
export type * from './models/DashboardPreference';
export type * from './models/PersonalLink';
export type * from './models/DashboardNote';
export type * from './models/Order';
export type * from './models/Invoice';
export type * from './models/Payment';
export type * from './models/FinancePostingPeriod';
export type * from './models/OperationalJournalEntry';
export type * from './models/Subscription';
export type * from './models/ExpensePlan';
export type * from './models/Expense';
export type * from './models/ExpensePayment';
export type * from './models/SalesBonusPolicy';
export type * from './models/BonusEntry';
export type * from './models/BonusRelease';
export type * from './models/ProductBonusPool';
export type * from './models/PayrollRun';
export type * from './models/SalaryLine';
export type * from './models/MailAccount';
export type * from './models/MailProviderConnection';
export type * from './models/EmailThread';
export type * from './models/EmailMessage';
export type * from './models/EmailAttachment';
export type * from './models/MailDeliveryLog';
export type * from './models/EmailRecipient';
export type * from './models/MessengerChannel';
export type * from './models/MessengerChannelMessage';
export type * from './models/MessengerChannelMessageAttachment';
export type * from './models/MessengerChannelReadState';
export type * from './models/MessengerDirectThread';
export type * from './models/MessengerDirectMessage';
export type * from './models/MessengerDirectMessageAttachment';
export type * from './models/MessengerDirectThreadReadState';
export type * from './models/InAppNotification';
export type * from './models/NotificationEvent';
export type * from './models/NotificationRule';
export type * from './models/NotificationJob';
export type * from './models/NotificationDelivery';
export type * from './models/Partner';
export type * from './models/PartnerReferralTerms';
export type * from './models/PartnerCommissionPolicyRow';
export type * from './models/PartnerAccrual';
export type * from './models/PartnerPayoutBatch';
export type * from './models/PartnerServiceTerm';
export type * from './models/Project';
export type * from './models/ProjectKickoffChecklistItem';
export type * from './models/Product';
export type * from './models/ProductAccessSlotBinding';
export type * from './models/Extension';
export type * from './models/Department';
export type * from './models/EmployeeDepartment';
export type * from './models/Role';
export type * from './models/Permission';
export type * from './models/RolePermission';
export type * from './models/Invitation';
export type * from './models/ReportExportJob';
export type * from './models/ReportSchedule';
export type * from './models/SavedReportView';
export type * from './models/SupportTicket';
export type * from './models/SystemListOption';
export type * from './models/WorkSpace';
export type * from './models/Task';
export type * from './models/TaskLink';
export type * from './models/TaskChecklist';
export type * from './models/TaskChecklistItem';
export type * from './models/TaskBoardStage';
export type * from './models/RecurringTaskTemplate';
export type * from './models/ProductTechnicalProfile';
export type * from './models/TechnicalAsset';
export type * from './models/TechnicalEnvironment';
export type * from './commonInputTypes';
