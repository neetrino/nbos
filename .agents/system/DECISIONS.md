...
## D-013 — NBOS activates security-review and code-review

Status: accepted

Context: NBOS is a Size C production monorepo with auth, payments, tenants, and multi-app surfaces. Library membership alone does not make Skills discoverable.

Decision: Activate `security-review` and `code-review` by moving them to `.agents/skills/` and marking them `active` in the catalog. Keep `react-performance-review` and `ui-accessibility-review` in the library until a later evaluation.

Consequences: Agents can discover the two review workflows. Profile `nbos` records the intended composition.
