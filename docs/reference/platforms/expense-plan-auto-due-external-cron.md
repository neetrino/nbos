...
If you intentionally run the job **inside** the API process, see `.env.example`:

- `SCHEDULER_EXPENSE_PLAN_AUTO_DUE_ENABLED`
- `SCHEDULER_EXPENSE_PLAN_AUTO_DUE_CRON`

Default is **off**; set timezone explicitly (`TZ`) if wall-clock matters. Production teams often still prefer **external** cron for isolation and predictable scaling.
