...
- Only the BFF refresh path writes that cookie (canon §13), including the expire-on-401 write. Watch `auth.refresh_reuse_detected` and `auth_sessions.revoke_reason='reuse_detected'`: a steady stream there means a second writer is replaying superseded refresh tokens, not a real token theft.
