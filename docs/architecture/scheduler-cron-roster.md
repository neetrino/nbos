...
16. 🖐️ **WhatsApp-группа продукта** — не cron. Позже: только кнопка, убрать воркер / cron / очередь.  
    Стоит в `todo.md`. `SCHEDULER_WHATSAPP_PRODUCT_GROUPS_RECONCILE_ENABLED`
...
### 16 — группа без cron и без очереди

Продукт сохраняется сразу, платформу не блокируем. Без группы команда не работает — сидит и ждёт. Нет группы — жмут ещё раз через ~10 с, если кнопка даёт.

Cron тут не нужен. Он только подбирал зависшие фоновые джобы.

Как должно быть:

1. Создали продукт — быстро, без WhatsApp. Группа = «нет».
2. Кнопка «Создать группу».
3. Пока идёт запрос — кнопка залочена, «создаём…».
4. Успех — чат на карточке.
5. Ошибка или таймаут ~25 с — «не создалась», кнопка снова через 10 с. Второй клик = новая попытка.
6. Ни cron, ни планировщик. Ретрай кликом.

Сейчас кнопка кладёт задачу в очередь и сразу отпускает — непонятно, ждать или жать ещё. Срез в `todo.md`: убрать cron, воркер и очередь, оставить только ручной запуск кнопкой.
