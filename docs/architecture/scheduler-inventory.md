...
| `invoice-card-reminders`            | POST `/scheduler/invoice-card-reminders`                                                | daily              | API             |         minutes | official-request + 5-day window | WhatsApp / notifications | **high** | EXTERNAL SIDE EFFECT |
...
| `whatsapp-product-groups-reconcile` | **removed** — do not batch-ensure groups for products without a binding (migrated data) |
