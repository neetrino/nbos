...
- позволяет держать критичный runtime под контролем команды (Gateway + инфраструктура), не смешивая его с доменным кодом NBOS.
...
NBOS не должен зависеть напрямую от WAHA, Whapi, Wazzup, Wappi или Evolution. На практике изоляция WAHA обеспечивается **WhatsApp Gateway** (см. `06-WhatsApp-Gateway-NBOS-Boundary.md`); модули NBOS зависят от стабильного API Gateway, а не от REST WAHA.
...
1. **Gateway** (реализация транспорта) получает файл или media URL от WAHA.
2. Байты передаются в NBOS по **согласованному защищённому контракту** (server-to-server), либо NBOS инициирует загрузку у Gateway; доменный метод адаптера по смыслу остаётся `downloadAttachment` на границе NBOS↔Gateway, а не прямой вызов WAHA из бизнес-модулей.
3. Drive в NBOS создаёт `File Asset`.
