...
>
> **Runtime:** app-shell Active Call Screen (centered modal). SSE `call.started` / `call.answered` / `call.finished`; snapshot via `GET /crm/calls/:id/screen` (includes `noteVersion`). Note after end (`PATCH /crm/calls/:id/note` with `expectedNoteVersion`; 409 keeps the local draft). Popup is not the source of call state.
>
