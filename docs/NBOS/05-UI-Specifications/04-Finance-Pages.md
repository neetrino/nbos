...
**Заголовок карточки (display title):** коммерческое название по каскаду (см. §2.2.1) — строка над суммой, меньшим шрифтом. `Invoice.code` — вторичная строка, когда не является заголовком.

| Элемент             | Описание                                                    |
| ------------------- | ----------------------------------------------------------- |
| Сумма               | Крупный шрифт, **основной** элемент карточки (не заголовок) |
...
Сортируемые колонки: display title (§2.2.1), `Invoice.code`, сумма, клиент, проект, тип, стадия, дата выставления, дата оплаты, дней просрочки.
...
- Это кнопка писем «не оплатили» (overdue). Первое письмо «в течение 5 дней» уходит кроном в день оплаты / позднего выставления, не этой кнопкой.
...
- Display title (§2.2.1), `Invoice.code` (вторично, если не заголовок), дата, сумма, статус
...
- **Subscription (левая колонка, три строки)** — сверху коммерческое **`Subscription.name`** (клик → sheet); ниже — название **проекта** (вторичная строка); снизу — ряд иконок (тип, billing day, partner, overdue/pending). Справа — кнопка **статуса** (меню смены; без «Open details»). Цвет полосы слева = status. `SUB-[SEQ]` — идентификатор, не primary label строки
