...
Форма — общий create для Tasks и `/quick/task`: title, description, outlined Assignee + Deadline; Files / Checklists / Project в футере. Поиск проекта открывается поверх карточки, не растягивает её. Чеклист открывается отдельной карточкой поверх задачи (карточка задачи не увеличивается и под затемнением неактивна, включая крестик задачи): клик по затемнению, крестику чеклиста или Save прячет слой и оставляет черновик на форме. Fork отдельного quick-only UI запрещён.

Карточка остаётся центрированным floating dialog (`mobileSheet={false}`), чтобы список проекта мог появиться над формой. Это та же форма, что открывается из Tasks; отдельный дизайн не создаётся.
