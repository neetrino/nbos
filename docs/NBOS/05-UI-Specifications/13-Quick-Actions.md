...
Форма — общий create для Tasks и `/quick/task`: title, description, outlined Assignee + Deadline; Files / Checklists / Project в футере. На desktop поиск проекта — popover над карточкой, чеклист — отдельная карточка поверх задачи (карточка не увеличивается и под затемнением неактивна, включая крестик задачи): клик по затемнению, крестику слоя или Save прячет слой и оставляет черновик на форме. На mobile (`max-md`) форма использует общий Dialog bottom sheet: выезжает снизу, свайп закрывает шит. Чеклист и поиск проекта на телефоне открываются слоем внутри шита, не отдельным centered modal. Fork отдельного quick-only UI запрещён.

Desktop — центрированный floating dialog. Mobile — тот же `QuickCreateTaskDialog` с default `mobileSheet` (bottom sheet). Отдельный дизайн не создаётся.

На desktop тот же компонент использует обычное modal представление. Quick Task в первую очередь оптимизируется под телефон, но route не должен искусственно ломаться на desktop.
