...
Пустой запрос показывает последние **открытые сущности** из этого браузера (localStorage), не последние строки поиска. Выбор результата из поиска или из Recent ставит карточку в начало списка. Поиск по тексту работает как раньше после ввода 2+ символов.
