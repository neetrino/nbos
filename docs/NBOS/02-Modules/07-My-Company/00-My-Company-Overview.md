...
Вкладка **General** позволяет сотруднику править свои личные поля (имя, телефон, Telegram, день рождения, ATS SIP ID, **фото профиля**) **без** права `COMPANY:EDIT`. Фото загружается файлом (JPEG / PNG / WebP) в Drive; в `Employee.avatar` хранится стабильный URL `/api/employees/:id/avatar`. Рабочий email, найм, роль, уровень, статус и отделы меняет только HR. Канон полей: `02-Team-Employees.md`.
