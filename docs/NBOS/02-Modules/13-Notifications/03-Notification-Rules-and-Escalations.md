...
| ----------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `invoice.created`                                                 | Invoice card created                                                                                                                                                          | Does not send to accountant. Official request fires on entering `Awaiting Payment`, not on create.                                                                                                                                                                                                                                                                                                                                                                                               |
| `finance.invoice.payment_reminder_due`                            | Subscription invoice unpaid; `notifications_enabled`; Tax official request sent when Tax; Yerevan day in `[anchor, dueDate]` (pay day, or official-issue day if we were late) | WhatsApp «оплатите в течение 5 дней» to Product WhatsApp Group. Catch-up if cron missed the anchor day. D-10 / D-2 (`dueDate − 10` / `− 2`) are retired.                                                                                                                                                                                                                                                                                                                                         |
| `finance.invoice.payment_reminder_d10` / `_d2`                    | **Retired offsets.** Historical jobs may still deliver.                                                                                                                       | —                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| `finance.invoice.official_request_due`                            | Tax card in `Awaiting Payment`; official request not sent                                                                                                                     | Catch-up: enqueue accountant WhatsApp via the same persist hook if auto-send on Awaiting entry failed. No `NotificationJob`. Primary trigger is entering Awaiting, not `dueDate`. Manual money-status change waits for that first send and shares its idempotency key with `Send` so a stale click cannot duplicate the group message. Client payment reminders stay separate and, for Tax, require official request already sent.                                                                                                                                 |
| `invoice.overdue` / `finance.invoice.overdue_reminder_w1` / `_w2` | `moneyStatus = OVERDUE`; Subscription or Client Service; `notifications_enabled`; Tax official request ready when Tax; Product WhatsApp group present                         | **No auto client send.** Finance presses **Send overdue reminders** after bank reconcile (`GET/POST /finance/invoices/overdue-reminders/*`). One letter per invoice per run: wave 1 if none sent, wave 2 if wave 1 was sent ≥ **2** Yerevan calendar days ago. After wave 2 stop. Copy asks the client to write if payment is blocked (transfer delay, etc.) so Finance can wait. Idempotency: `NotificationJob` key `invoice_overdue_reminder:w{n}:{invoiceId}`. Ops In-App/Telegram unchanged. |
...
Scheduler: `POST /api/scheduler/invoice-card-reminders` (Yerevan `asOf`) — Tax official-request **and** subscription 5-day payment window. Overdue client waves are **manual**, not cron.
...
| `subscription.payment_reminder` | Subscription unpaid; window `[dueDate − 5, dueDate]` | Client WhatsApp «в течение 5 дней» via `finance.invoice.payment_reminder_due` |
