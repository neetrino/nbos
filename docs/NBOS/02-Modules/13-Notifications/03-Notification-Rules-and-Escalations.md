...
| `invoice.awaiting_payment` / `finance.invoice.payment_reminder_d10` / `_d2` | Unpaid subscription **or Client Service** invoice; `notifications_enabled`; Tax official request ready when Tax; Yerevan calendar day = `dueDate − 10` or `dueDate − 2` | WhatsApp client payment reminder to **Product WhatsApp Group** (`subscription.productId`, else `ClientServiceRecord.productId`); language = `Subscription.reminderLanguage` or `ClientServiceRecord.reminderLanguage`. Idempotency: one job per `(invoiceId, offsetDays)` |
...
| `invoice.overdue`                                                           | Payment не получен после due date (ops alerts)                                                                                                    | In-App/Telegram Finance (client WhatsApp pre-due offsets above; not daily client spam)                                                                                                                                                    |
...
| `subscription.payment_missing`  | Invoice overdue (ops)           | Finance alert; client path is pre-due offsets only       |
