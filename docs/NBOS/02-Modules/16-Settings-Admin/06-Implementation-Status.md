...
- **API inventory:** `GET /platform/lifecycle/trash-inventory` (`SETTINGS VIEW`).
- **Retention rules:** `GET /platform/lifecycle/retention-rules` — registry + env `PLATFORM_TRASH_RETENTION_DAYS_*`.
- **Unified purge:** `POST /platform/lifecycle/purge/run` (`SETTINGS DELETE`); scheduler `POST /scheduler/platform-trash-purge`.
