...
| `Appearance`                | Общий фон платформы (WebP light/dark), не личные обои      |
...
Платформенный chrome, не бренд юрлица и не персональная тема сотрудника.

- одно фото на всю компанию, два слота: light и dark;
- загрузка только WebP, до 400 KB, длинная сторона ≤ 2560 px, короткая ≥ 1280 px, без анимации;
- право записи: `SETTINGS` + `EDIT`;
- файл хранится вне Drive, отдаётся стабильным versioned URL с длинным HTTP-кешем;
- карточки, сайдбар и док остаются плотными. Фон виден в зазорах canvas.

Тема light/dark остаётся личной (`next-themes`). Appearance не заменяет Settings General.
