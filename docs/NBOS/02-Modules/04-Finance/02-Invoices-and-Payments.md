...
## Доменные Invoice — решение 2026-09-17

**Утверждено, реализация ожидается.** [Полный канон](08-Domain-Purchase-and-Connection.md).

В Invoice Board сохраняется свободный путь и добавляется «Покупка домена» с обычным dropdown Products. Не добавлять Deal/Client tabs и покупки без Product. Клиент/Project наследуются. Карточка сервиса создаётся или переиспользуется при создании счёта, до Payment.

Несколько доменов одним submit дают отдельный Invoice на каждый Client Service; текущая одиночная связь сохраняется. Official request, сумма и название домена остаются отдельными для каждого счёта. Частичный сбой восстанавливает только недостающие счета.

После полной оплаты — идемпотентное создание/поиск Expense и недостающая подготовка. Свободный счёт не запускает доменный flow по тексту. Сохраняются действующие permissions и tax/official/notification правила. Оплата домена не переводит Deal в Won.
