...
Можно выбрать сразу несколько месяцев: каждый месяц — **отдельная** карточка с `amount` периода, без умножения. Одна транзакция; если второй месяц не проходит, откатываются все. Не больше 12 карточек за один запрос.

Только статус `Active`. Сумма, налог, `dueDate` и money status — те же правила, что у billing cron. Окно: от месяца `billing_start_date` до текущего месяца Еревана + 12. Карточка, которая пересекается с уже покрытыми месяцами (или с другой выбранной в этой пачке), не создаётся. Срок (`termMonths`) не должен быть исчерпан. Late-delivery pause блокирует выставление, как и cron.

Точки входа: вкладка **Invoices** в Subscription sheet и доска Invoices при фильтре по `subscriptionId`. Выбор месяцев — только из доступных; сумму и due date вручную не задают. Для `Yearly` / `Custom` выбирается старт периода; пересекающиеся старты в списке недоступны.
