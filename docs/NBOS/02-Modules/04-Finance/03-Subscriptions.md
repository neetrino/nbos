...
**Инвариант:** `amount` — единственный источник истины для денег. Сумма на `Invoice Card` = `amount` напрямую, без умножения. **Исключение:** разовое ручное выставление сразу за несколько периодов (см. ниже) — одна карточка с `amount × N` и покрытием `coverage_month_count × N`; частоту подписки это не меняет. `monthly_equivalent_amount` — только для MRR и сравнения подписок разной частоты; **никогда** не использовать для расчёта счёта, не вводить вручную и **не умножать обратно**, чтобы восстановить сумму контракта.
...
- **Изменение суммы** → на карточке подписки; выставленные месяцы остаются на сумме инвойса, новая ставка только на forecast / ещё не выставленных. Выбор месяца и `amount_history` — отдельный срез
...
### Ручное выставление карточки

Daily cron — основной путь. Finance также может вручную выставить `Invoice Card` за **непокрытые** месяцы покрытия:

- текущий месяц (сразу после создания активной подписки);
- будущие месяцы в окне (предоплата, до текущего месяца Еревана + 12);
- прошлый непокрытый месяц в окне (догон).

Можно выбрать сразу несколько **подряд** идущих стартов периода: система создаёт **одну** карточку. Сумма = `amount` периода × число выбранных стартов; покрытие = `coverage_month_count` подписки × то же число. Частоту подписки не меняем. Разрыв (сентябрь + ноябрь без октября) — отказ, без автозаполнения. Не больше 12 выбранных стартов за один запрос. Если карточку выставили ошибочно — отменяют её целиком и создают заново.

Только статус `Active`. Налог, `dueDate` и money status — те же правила, что у billing cron. Окно: от месяца `billing_start_date` до текущего месяца Еревана + 12. Карточка, которая пересекается с уже покрытыми месяцами, не создаётся. Срок (`termMonths`) должен покрывать всё окно. Late-delivery pause блокирует выставление, как и cron.

Точки входа: вкладка **Invoices** в Subscription sheet и доска Invoices при фильтре по `subscriptionId`. Выбор месяцев — только из доступных; сумму и due date вручную не задают. Для `Yearly` / `Custom` выбирается старт периода; пересекающиеся старты в списке недоступны. Бухгалтеру и клиенту уходит одно письмо с полной суммой и диапазоном месяцев покрытия.

### Изменение суммы подписки

`Subscription.amount` — ставка **следующих** выставок. Уже созданный `Invoice.amount` замораживается в момент выставления и при смене суммы подписки **не** переписывается.

При изменении `amount` на карточке подписки:

1. Указывается новая сумма за период
2. Уже выставленные Invoice Card (прошлые и текущий месяц, если карточка уже создана) остаются со своей суммой
3. Сетка на покрытых месяцах показывает эквивалент **того** инвойса: `invoice.amount / coverage_month_count`
4. Forecast / ещё не выставленные месяцы и новые карточки биллинга используют новый `amount` / `monthly_equivalent_amount`
5. `amount_history` и выбор месяца «с какого действует» — отдельный срез (ещё не реализован)
