...
| `prepaid_month_count`   | Число предоплаченных месяцев для `Custom` (2–60); для `Monthly` / `Yearly` не задаётся               |
...
| Значение  | Смысл                           | `prepaid_month_count`       |
| --------- | ------------------------------- | --------------------------- |
| `Monthly` | обычная ежемесячная оплата      | не задаётся                 |
| `Yearly`  | одна оплата сразу за 12 месяцев | не задаётся (покрытие = 12) |
| `Custom`  | предоплата на несколько месяцев | обязательно, от 2 до 60     |

Для `Custom` сумма карточки = `base_monthly_amount × prepaid_month_count`, покрытие = столько же месяцев. Для `Yearly` покрытие всегда 12 месяцев, сумма = `base_monthly_amount × 12`.
