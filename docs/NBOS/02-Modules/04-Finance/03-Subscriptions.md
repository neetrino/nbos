...
Daily cron — основной путь. Finance также может вручную выставить `Invoice Card` за **непокрытый** месяц покрытия:

- текущий месяц (сразу после создания активной подписки);
- следующий месяц (если карточку заводят в середине месяца);
- прошлый непокрытый месяц в окне (догон).

Только статус `Active`. Сумма, налог, `dueDate` и money status — те же правила, что у billing cron. Окно: от месяца `billing_start_date` до текущего месяца Еревана + 1. Карточка, которая пересекается с уже покрытыми месяцами, не создаётся. Срок (`termMonths`) не должен быть исчерпан. Late-delivery pause блокирует выставление, как и cron.

Точки входа: вкладка **Invoices** в Subscription sheet и доска Invoices при фильтре по `subscriptionId`. Выбор месяца — только из доступных; сумму и due date вручную не задают.
