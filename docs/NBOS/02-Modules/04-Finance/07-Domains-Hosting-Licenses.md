...
| `Invoice`  | только `WE_PAY`: до `renewal_date` < 2 месяцев → авто-создание `Invoice Card` (см. EXP-04); `dueDate` карточки = создание + **15** календарных дней (не `renewal_date`); ждём оплату клиента |
...
Порог «2 месяца» = граница стадии `Invoice` и совпадает с автоматизацией продления (`EXP-04`). `renewal_date` остаётся датой продления / оплаты провайдеру. Просрочка **карточки клиента** — через 15 дней после создания счёта (~45 дней запаса до продления). После `PAID` создаётся `Expense` в `DUE_NOW` (Pay now).
