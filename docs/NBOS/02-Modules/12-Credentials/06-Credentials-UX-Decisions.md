...
- **Implemented (2026-06-02):** Slice D bulk — `POST /credentials/bulk/archive|restore`, export selected; List/Tiles selection (hover-reveal → pinned after first pick). **Saved views:** localStorage `nbos:credentials:vault-saved-views` (save/apply/remove, max 15).
