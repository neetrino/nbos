...
Вкладка `Credentials` на странице Product показывает vault-записи, привязанные к продукту (скалярный `productId` **или** строка в `product_access_slot_bindings`), включая shared infra с `projectId = null`.

Ротация shared infra-аккаунта: перед сменой пароля смотреть обратный список привязок — все продукты, использующие этот аккаунт. Копии секрета запрещены именно потому, что ломают эту картину.
