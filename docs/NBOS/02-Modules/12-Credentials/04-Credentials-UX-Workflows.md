...
## Аккаунт регистратора из Client Service — решение 2026-09-17

**Утверждено, реализация ожидается.** [Канон доменного процесса](../04-Finance/08-Domain-Purchase-and-Connection.md).

В dropdown Credential карточки Client Service доступны выбор существующей записи и «Создать Credential» по общему inline-create паттерну. Новая запись сразу выбирается и связывается. Один аккаунт используется несколькими доменами/Products; пароль не копируется на каждый домен. URL хранится в Credential, отдельное обязательное поле ссылки оплаты не добавляется.

Из обычной Task открывается связанный Client Service поверх задачи. Специальные поля аккаунта/стоимости в Task не вводятся. Ссылка в открытом Expense становится доступна и при более позднем сохранении Credential на сервисе; оплаченная история не изменяется молча.

Контекстная связь не выдаёт reveal/copy grants. Общая карточка «DNS» не используется как новый фиктивный доступ: её legacy-связи после проверки заменяются явным способом подключения. Не удалять её или общие аккаунты автоматически.
