...
- для `DEV_ONLY` / `DEV_AND_MAINTENANCE` начисление создаётся сразу, но держится как `ACCRUED` до сдачи связанного product/extension (любой переход в `DONE`), затем становится `ELIGIBLE`;
- если product/extension закрывается как `LOST`, held-строки (`ACCRUED` + `subscriptionId`) становятся `CANCELLED`; `ELIGIBLE` / `IN_BATCH` / `PAID` не меняем; журнальная строка `PARTNER_ACCRUAL` реверсится (`REVERSED`);
