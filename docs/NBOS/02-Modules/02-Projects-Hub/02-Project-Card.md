...
| Company        | На карточке проекта — уникальные Company продуктов, каждая один раз. `Project.companyId` остаётся seed для новых продуктов и на карточке отдельно не показывается |
