...
Текущая встроенная Project Delivery Board v1 считается transitional UI. В новом каноне она заменяется отдельной `Delivery Board` page. Если позже нужен delivery block внутри Project page, он должен переиспользовать тот же `DeliveryBoardCore` с фильтром `projectId`, lazy-load и без дублирования логики.
