...
- На opened Delivery / General показываются **все** слоты профиля + **UNIVERSAL**; **обязательные** типовые слоты помечаются в UI (иконка). Для обязательного слота требуется **минимум одна** привязка; остальные — по желанию. Stage-gate блокирует только переход в **Done**, когда Domain или Hosting пусты (домен и хостинг закрываются на Transfer).
- Слот принимает **существующий** credential (`Link existing`) и `+ New`. Источники в picker: vault этого project + пул shared infra (`projectId = null`, категория совпадает с `allowedCategories` слота). Клик по полю открывает выпадающий список; `+` у названия слота создаёт новую запись.
- Один shared-аккаунт (`projectId = null`, vault scope `Secret`) может быть привязан к продуктам **разных** projects. Клиентский credential (`projectId` заполнен) — только внутри своего project.
- Один registrar-аккаунт клиента обслуживает **все** его домены: 10 доменов = 1 credential + N привязок, не 10 записей. Копировать секрет ради per-product детали запрещено.
...
- Значения входа/секреты не дублируются в карточке: слот указывает на **Credential**; создание и редактирование — через модуль Credentials и RBAC. Bind **не** выдаёт доступ и **не перетирает** непустой `Credential.productId`; у shared-записи `productId` остаётся `null`.
- Если у сотрудника нет прав на привязанный credential, слот показывает **имя + category + `Request access`**, без `login` / `url` / секретов. Пустым слот не выглядит: факт связи — рабочая информация.
- **Create from slot (`+ New`):** title новой credential-карточки = имя **родительского Product** — и с карточки продукта, и с карточки Extension (не имя доработки и не label слота). Category / slot badge остаётся источником «это Domain / Hosting / …»; имя отвечает на «для какого продукта». Пользователь может переименовать до сохранения.
...
- Вкладка `Credentials` на странице Product в Projects Hub показывает те же привязки, включая shared infra вне `projectId` продукта.
...
- обязательные поля;
- completed checklist instances как отдельные верхнеуровневые requirements;
...
- Product / Extension stage gate не должен проверять `checklist.item.key`;
- checklist instance сам решает, можно ли нажать `Complete checklist`;
- `Not Done` всегда требует причину;
- обязательные review-пункты нельзя оставить `Pending`;
- stage gate получает только один факт: checklist instance completed.

Это сохраняет stage gates простыми и позволяет иметь разные чеклисты для `WordPress`, custom code, app, extension и других product types без переписывания логики переходов.
