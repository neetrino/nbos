...
## 5.1. AI-акторы и артефакты (Phase 1)

External Agent не получает Drive-доступ как сотрудник и не имеет собственных `FileAssetGrant`. Его доступ полностью производный от задачи:

- агент видит артефакт только через `File -> Task` для задачи, которая уже разрешена его scope;
- артефакт, привязанный к неразрешённой задаче, отвечает тем же «resource not available», что и несуществующий, — существование файла не раскрывается;
- прикрепление артефакта идёт через ту же доменную загрузку, что и у сотрудника, и записывается с actor provenance;
- расширение видимости через link не даёт агенту новый scope: сначала проверяется задача, потом файл.

Канон: `02-Modules/21-AI-Platform/03-External-Agent-Access.md`.
