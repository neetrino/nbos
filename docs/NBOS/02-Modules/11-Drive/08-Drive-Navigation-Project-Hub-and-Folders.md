...
Project Library:
  ...
  └── Project files: файлы уровня Project shell, связанные только с PROJECT
```

Файлы только с `FileLink(DEAL)` **не обязаны** появляться в Project Library до появления линка `PROJECT`.
Файлы с `FileLink(PROJECT)` **и** `FileLink(DEAL/PRODUCT/TASK/INVOICE/CLIENT/…)` показываются в соответствующей виртуальной секции, а не в Project files.
...
| Project files         | Список                | только `FileLink(PROJECT)` без child/entity context links |
| Commercial / Deal     | Виртуальная секция    | `FileLink(DEAL)` + связь CRM                              |
| Products / Extensions | Виртуальная секция    | связанные Product / Extension + links                     |
...
**Project files — не общий fallback для неразложенных файлов.** Это файлы самого Project shell: общий kickoff, project brief, roadmap, общая handoff/checklist-запись, meeting recording по проекту в целом. Если файл имеет более конкретный контекст (`DEAL`, `PRODUCT`, `TASK`, `INVOICE`, `CONTACT`, `COMPANY`, …), он принадлежит соответствующей секции хаба.
...
3. **PROJECT scoped folders + hub sections + Project files**.
