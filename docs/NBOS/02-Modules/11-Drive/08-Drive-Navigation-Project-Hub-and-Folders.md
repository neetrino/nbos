...
Project Library:
  ├── Folders: PROJECT scoped tree + project shell files на корне дерева
...
| Зона                  | Тип                   | Источник                                                        |
| --------------------- | --------------------- | --------------------------------------------------------------- |
| Project folders       | Scoped tree `PROJECT` | `DriveFolder` + `DriveFolderItem`; project shell files на корне |
...
**Project shell files** показываются на корне `Folders`, не отдельной вкладкой. Это файлы с `FileLink(PROJECT)` без links на `DEAL` / `PRODUCT` / `TASK` / `INVOICE` / `CONTACT` / `COMPANY` / … . Отдельной кнопки **All project files** в UI нет — агрегат всех `FileLink(PROJECT)` не нужен обычному пользователю.
...
3. **PROJECT scoped folders + hub sections** (project shell files at folder root).
