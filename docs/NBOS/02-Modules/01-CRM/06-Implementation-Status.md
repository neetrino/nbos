...
- **API:** `GET /crm/leads`, `GET /crm/deals` with `?scope=active|trash` (default active); `DELETE :id` → move to Trash; `POST :id/restore`.
- **Web:** Leads/Deals list views with scope switch; sheets — Move to Trash / Restore; trash rows read-only in sheets (no stage change).
...
- **Permanent delete** before retention — admin API + confirmation (platform backlog).
...
- Operational delete = **Trash-first** (`09-Entity-Lifecycle-Standard.md`). Hard purge only via retention job or future admin permanent API.

## API routes (lifecycle)

- `crm/leads`, `crm/deals` — list + `scope`; `DELETE` → Trash; `POST :id/restore`.
