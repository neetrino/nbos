...
| Area                 | Status    | Action                                                                                                                                                                                                                                                                                                     |
| -------------------- | --------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Mail backend module  | `PARTIAL` | **2026-04-29:** `POST …/drafts`, queue, **`finalize-send-stub`** (+ in-app notify on FAILED), `reset-failed-to-draft`, `cancel`, **`POST …/accounts/:id/sync-stub`** (+ in-app notify), list `unreadOnly` + `needsLinkOnly` + **`q`**, **`PATCH …/threads/:id`**, mark-read + audits; без SMTP/sync/worker |
...
| Notifications events | `PARTIAL` | **2026-04-29:** после `sync-stub` и **finalize-send-stub (FAILED)** — in-app актору (+ владелец ящика при отличии); строки в `in_app_notifications` (Prisma); health/inbound/real sync — ещё `MISSING`                                                                                                     |
