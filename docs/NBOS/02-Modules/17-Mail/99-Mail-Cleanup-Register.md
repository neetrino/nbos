...
| Notifications events | `PARTIAL` | **2026-04-29:** in-app после `sync-stub`, **queue (DRAFT→QUEUED)**, **finalize-send-stub (FAILED)**, **cancel**, **reset-failed-to-draft** — актору (+ владелец ящика при отличии); `in_app_notifications` (Prisma); health/inbound/real sync — ещё `MISSING`               |
