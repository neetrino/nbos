...
| Area                 | Status    | Action                                                                                                                                                                                                                                                                                                                                                                                  |
| -------------------- | --------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Mail backend module  | `PARTIAL` | **2026-04-29:** `MailOutboundMutationService` (drafts, queue, finalize-stub, cancel, reset-failed) + in-app notify; **`POST …/accounts/:id/sync-stub`**; **`GET …/accounts/health-summary`** (scoped thread aggregates); list `unreadOnly` + `needsLinkOnly` + **`q`**, **`PATCH …/threads/:id`**, mark-read + audits; без SMTP/sync/worker                                             |
...
| Mail UI              | `PARTIAL` | **2026-04-29:** inbox (stub **sync** per mailbox, All / Unread / **Needs link**, **subject search** debounced), thread (needs-link PATCH, **Queue**/Cancel draft, Finalize/Cancel queued, **Reset failed→draft**, reply, mark read); **2026-04-30:** per-mailbox **health summary** (thread/unread/needs-link counts + last error); нет реальной отправки/settings/live provider health |
...
| Notifications events | `PARTIAL` | **2026-04-29:** in-app после `sync-stub`, **outbound draft**, **queue (DRAFT→QUEUED)**, **finalize-send-stub (FAILED)**, **cancel**, **reset-failed-to-draft** — актору (+ владелец ящика при отличии); `in_app_notifications` (Prisma); health/inbound/real sync — ещё `MISSING`                                                                                                       |
