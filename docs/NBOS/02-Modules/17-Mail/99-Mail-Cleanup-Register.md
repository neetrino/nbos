...
| Area                 | Status    | Action                                                                                                                                                                                                                                                               |
| -------------------- | --------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Mail backend module  | `PARTIAL` | **2026-04-29:** `POST …/drafts`, queue, `finalize-send-stub`, `reset-failed-to-draft`, `cancel`, **`POST …/accounts/:id/sync-stub`** (+ in-app notify MVP), list `unreadOnly` + `needsLinkOnly`, **`PATCH …/threads/:id`**, mark-read + audits; без SMTP/sync/worker |
...
| Mail UI              | `PARTIAL` | **2026-04-29:** inbox (stub **sync** per mailbox, All / Unread / **Needs link**), thread (needs-link PATCH, **Queue**/Cancel draft, Finalize/Cancel queued, **Reset failed→draft**, reply, mark read); нет реальной отправки/settings/health                         |
