...
- **API:** `POST /mail/threads/:id/delete` → move to Trash (not hard delete); `POST …/restore`; list `?scope=active|trash`.
- **Web:** Mail sidebar **Trash** folder; detail — Move to Trash / Restore; active inbox excludes trashed threads.
...
- `mail/threads` — `scope`; `POST …/delete` (trash); `POST …/restore`.

## Related code

- API: `apps/api/src/modules/mail/mail-thread-trash.ops.ts`, `mail-trash-purge.ops.ts`
