...
- Client payment/overdue/subscription-window (and CSR invoices with Product): `resolveInvoiceProductWhatsAppGroup` → `resolveClientDestination(productId, FINANCE)` (explicit FINANCE else WORK) → persist Core SYSTEM OUTBOUND → Slice 8 `core_client_send`.
...
- Durable Client Messenger history exists for Product payment reminders (SYSTEM outbound). Official accountant send is unchanged.

Classification: destination lookup Slice 9 (`VERIFIED`); reminder persist/send Slice 10 (`VERIFIED`); outbound worker `REUSE` (`core_client_send`); leftover `payment_reminder`/`overdue_reminder` kinds are skipped; accountant group must not be collapsed into Product WORK/FINANCE.
...
| Finance reminder destination/delivery                        | `MIGRATE` + `REUSE` worker                         | Slice 9: destination via `resolveClientDestination(productId, FINANCE)` (WORK fallback). Slice 10 (`VERIFIED`): persist Core SYSTEM + `core_client_send`. Official: `accountingGroupChatId`. |
...
**Slice 8:** that consume path exists for MESSENGER Client conversations (`VERIFIED`). Product purpose bindings are Slice 9 (`VERIFIED`). Finance reminder persist/send is Slice 10 (`VERIFIED`).
...
**Slice 10:** evidence `30-Slice-10-Finance-Support-Routing.md`. Status `VERIFIED`. FINDING-S10-01…08 closed. Finance reminders persist Core then `core_client_send`. Support Ticket sources are references. Attention is additive (Hub Maintenance formula, not `Product.status DONE`). No DROP. Slice 11 not started (hard stop).
