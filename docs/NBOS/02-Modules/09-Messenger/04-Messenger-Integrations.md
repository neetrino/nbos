...
**NBOS Mail** (`docs/NBOS/02-Modules/17-Mail/*`) — отдельный модуль для **email inbox** (Gmail/IMAP), не часть Messenger.

| Messenger                         | Mail (`17-Mail`)                                  |
| --------------------------------- | ------------------------------------------------- |
| `Conversation` / `Message`        | `EmailThread` / `EmailMessage`                    |
| WebSocket live, чат UI            | REST + inbox UI `/mail`, sync jobs                |
| WhatsApp / CRM Inbox как чаты     | SMTP send, IMAP receive, threading по RFC headers |
| Вложения → Drive в контексте чата | Вложения → Drive как `FileAsset` + mail metadata  |

Support intake **по email** описан в Mail (`17-Mail/04-Mail-Integrations.md`, Support): письмо может породить Ticket, дальше коммуникация может продолжаться в Messenger — это разные каналы и разные сущности.
