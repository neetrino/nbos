...
| Merge / dedupe | Contact = who (1:N Leads). Lead = inquiry. Deal = opportunity. Intake attach by phone/email/Instagram; no auto-merge without a shared id (ATS exact-phone attach to open non-SQL Lead stays). Lead merge via wizard + Trash + `mergedIntoId`. Deal merge last, strict guards; default do not merge. Seller merges only cards they own; Head of Sales / CEO any; Marketing read-only. See `02-Modules/01-CRM/07-Lead-and-Deal-Merge.md`. |
