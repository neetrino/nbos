...
- **Асинхронно:** BullMQ. Пример: по расписанию (cron) — создание инвойсов по подпискам, смена стадий инвойсов, ежемесячные planned expenses, проверка SLA, выпуск доступных project bonus pool сумм в payroll. Очереди позволяют повтор при сбоях и не блокируют API.
...
- **Redis:** Upstash.
- **Файлы:** Cloudflare R2.
- **CI/CD:** GitHub Actions выполняет CI; production migration и выборочный Coolify deploy выполняются вручную после зелёного CI. С машины оператора: `pnpm deploy:prod`, цепочка migrate+deploy — `pnpm release:prod` (`docs/deploy.md` §5.3–5.4). Auto Deploy остаётся OFF.
