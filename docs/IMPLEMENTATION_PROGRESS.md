...
- **Payroll / bonus:** **Bonus board** (`GET /api/bonus` + stats); **`GET /api/bonus/projects/pools`** project roll-ups (pipeline / paid / clawback from `bonus_entries`); **Payroll runs** + **APPROVED** → **`Expense`**; **`ExpensePayment`** ↔ salary lines + **`payroll_runs.total_paid`**; **Employee Wallet** read-only (`GET /api/me/wallet`, `/finance/wallet`); payroll detail **`journal`** + **`auditTrail`** (`audit_logs` entity `PAYROLL_RUN`: `CREATED`, `STATUS_CHANGED`). **Next:** optional dedicated NBOS pool entity when schema lands; deeper payroll reporting when roadmap calls for it.
...
| 2026-04-29    | Payroll audit trail (`audit_logs`)             | `CREATED` when `createdById` set + `STATUS_CHANGED` in txn with status update; `loadPayrollRunAuditTrail`; Finance **Audit trail** + `payrollAuditActionLabel`; PATCH passes `actorUserId`                                                                 | Vitest `payroll-run-audit-trail` + payroll findById mocks; api/web `tsc` + ESLint                         | NBOS pool entity; payroll reporting depth         |
