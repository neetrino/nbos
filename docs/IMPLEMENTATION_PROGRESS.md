...
| 2026-04-29    | Mail & Notifications | Mail MVP (accounts, threads, outbound stub, cancel/reset, needs-link, sync-stub); in-app notify on stub sync; Topbar feed → API (JWT-scoped); **`in_app_notifications`** Prisma + `NotificationService` on DB. |
