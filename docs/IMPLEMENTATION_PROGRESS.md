...
Статус: этапы 0–3 готовы; 4 почти закрыт; 5 в работе — live logout/login двух тестовых пользователей пройден (язык из БД, cookie не протекает). Срез не в IMPLEMENTATION_DONE. Открыты live token-expiry и production rollout. [Канон](NBOS/01-Platform-Overview/07-Interface-Localization.md), [план](implementation/I18N-IMPLEMENTATION-PLAN.md). Этапы 6–7 не начаты.
...
- **Телефония / ATS (звонки):** Call core + Active Call Screen + CRM CALL activity + recording FileAsset/playback + click-to-call с `Idempotency-Key` / `AtsCallIntent` (at-most-once) + atomic webhook uid/sparse/monotonic state в runtime. Осталось по `08-Calls` / Cleanup Register §C5: persisted `projectId` / `productId` on Call и history reconcile. DID / транскрипт / импорт Bitrix — later. — **L**
