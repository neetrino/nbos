...
- **Телефония / ATS (звонки):** Call core + Active Call Screen + CRM CALL activity + recording FileAsset/playback + click-to-call с `Idempotency-Key` / `AtsCallIntent` (at-most-once) + atomic webhook uid/sparse/monotonic state в runtime. Осталось по `08-Calls`: history reconcile. DID / транскрипт / импорт Bitrix — later. — **L**
