...
| Current task         | Payroll audit trail (NBOS) when canon confirms; **run journal v1** from timestamps shipped; expense plan **optional cron** env-gated; bonus pool roll-ups shipped. |
| Status               | Open                                                                                                                                                               |
...
- **Payroll / bonus:** **Bonus board** (`GET /api/bonus` + stats); **`GET /api/bonus/projects/pools`** project roll-ups (pipeline / paid / clawback from `bonus_entries`); **Payroll runs** + **APPROVED** → **`Expense`**; **`ExpensePayment`** ↔ salary lines + **`payroll_runs.total_paid`**; **Employee Wallet** read-only (`GET /api/me/wallet`, `/finance/wallet`); payroll detail **`journal`** (created / approved / closed from durable fields). **Next:** full audit trail for payroll; optional dedicated NBOS pool entity when schema lands.
...
| Finance                     | Yes        | Yes          | Partial     | Partial | Orders/invoices/payments/subscriptions/expenses + reconciliation; partner filters; expense backlog + **Closed** + ledger, CSV, guards; **Expense Plan** (sidebar **Expense plans**) + **manual + auto due Plan→Card** + **NBOS board** (`activeBoard`); **Bonus board** + **`GET /api/bonus/projects/pools`** + **Bonus pools** (Finance tab + sidebar) + **Payroll runs** (detail **journal v1**) + **My wallet** (`GET /api/me/wallet`); optional `expense_plan_id`; tab title helpers; legacy `/expenses` redirect |
