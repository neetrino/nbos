...
| Current task         | Optional **auto** card from plan (`autoGenerate` + runner agreement); **payroll** depth—larger slices need product/schema agreement. |
| Status               | Open                                                                                                                                 |
...
- **Expenses:** backlog + **Closed** (`/finance/expenses/closed`, paid list); partial payments + delete; ledger sync/guards; CSV; **Expense Plan** + **manual card**; **Expense Board** (`activeBoard`). **Next:** auto card from plan; payroll.
...
| Finance                     | Yes        | Yes          | Partial     | Partial | Orders/invoices/payments/subscriptions/expenses + reconciliation; partner filters; expense backlog + **Closed paid list** + ledger, CSV, guards; **Expense Plan** + **manual Plan→Card** + **NBOS board** (`activeBoard`, five lanes); optional `expense_plan_id`; tab title helpers; legacy `/expenses` redirect |
...
1. **Expense automation** — optional **auto** card from plan when product agrees (`autoGenerate` + safe runner/cron).
...
- Expenses: partial payments, backlog, **Closed list**, ledger, **Plan + manual card + NBOS board + activeBoard**—shipped; **next = optional auto + payroll** (by agreement).
