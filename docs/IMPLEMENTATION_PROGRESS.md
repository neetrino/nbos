...
| Finance                     | Yes        | Yes          | Partial     | Partial | Orders/invoices/payments/subscriptions/expenses + reconciliation; partner filters; expense backlog + **Closed** + ledger, CSV, guards; **Expense Plan** + **manual + auto due Plan→Card** + **NBOS board** (`activeBoard`); **Bonus board** + **`GET /api/bonus/projects/pools`** + **Bonus pools** tab + **Payroll runs** + **My wallet** (`GET /api/me/wallet`); optional `expense_plan_id`; tab title helpers; legacy `/expenses` redirect |
