...
| Current task    | Scheduled reports model            |
...
| Phase 6 — Control layer             | In progress | ~64%     | Reports catalog, CSV export writer and Dashboard Control Center/projection/preferences shipped; schedule depth next |
...
2. Next slice: scheduled reports model (owner, recipients, format, next run, last run, status and failure state).
3. Later Phase 6: BullMQ export worker wiring, data-quality warnings, Dashboard role defaults/personal links and KPI/Scorecard depth.
