...
| Current task         | **Payroll** depth (bonus pool / wallet projections); **Bonus board** API-backed; **Payroll runs** + expenses + **line paid ↔ expense ledger**; expense plan auto via **Scheduler** POST. |
| Status               | Open                                                                                                                                                                                     |
...
| Payroll / bonus pools / operational journal                                                                                              |             ~45% |
...
- **Payroll / bonus:** **Bonus board** (`GET /api/bonus` + stats); **Payroll runs** + **APPROVED** → **`Expense`** per line; **`ExpensePayment` add/delete** syncs `salary_lines.paid_amount` / `remaining_amount` / status (`APPROVED`→`PARTIALLY_PAID`→`PAID`) and **`payroll_runs.total_paid`**. **Next:** bonus pool / wallet read-model per NBOS.
...
| Finance                     | Yes        | Yes          | Partial     | Partial | Orders/invoices/payments/subscriptions/expenses + reconciliation; partner filters; expense backlog + **Closed** + ledger, CSV, guards; **Expense Plan** + **manual + auto due Plan→Card** + **NBOS board** (`activeBoard`); **Bonus board** + **Payroll runs** (**APPROVED** → **`Expense`**; **payments** → salary line + run `total_paid`); optional `expense_plan_id`; tab title helpers; legacy `/expenses` redirect |
