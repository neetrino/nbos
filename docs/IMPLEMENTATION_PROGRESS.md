...
| Current task         | Finance ops polish / scheduler wiring; deeper payroll journal when canon confirms; **Project bonus pool roll-ups** shipped (aggregates from bonus entries). |
| Status               | Open                                                                                                                                                        |
...
- **Expenses:** backlog + **Closed**; partial payments + delete; ledger sync/guards; CSV; **Expense Plan** + **manual card** + **auto due** (finance API + **scheduler** POST for cron); **Expense Board** (`activeBoard`). Payroll-linked expenses also roll up to **salary_lines** + **payroll_runs.total_paid**.
...
- **Payroll / bonus:** **Bonus board** (`GET /api/bonus` + stats); **`GET /api/bonus/projects/pools`** project roll-ups (pipeline / paid / clawback from `bonus_entries`); **Payroll runs** + **APPROVED** → **`Expense`**; **`ExpensePayment`** ↔ salary lines + **`payroll_runs.total_paid`**; **Employee Wallet** read-only (`GET /api/me/wallet`, `/finance/wallet`). **Next:** external cron for plan auto-due; optional dedicated NBOS pool entity when schema lands.
