...
| 2026-04-29    | Mail & Notifications | Mail MVP + **`q`**; **`GET …/accounts/health-summary`**; **`MailDeliveryLog`** + delivery-log UI; outbound **`MailOutboundSendMutationService`**; in-app: stub sync, outbound pipeline, **thread needs-link flagged/cleared**; **`in_app_notifications`** + Topbar API. |
