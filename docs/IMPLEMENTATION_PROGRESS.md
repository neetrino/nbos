...
| Current task  | Phase 5 Documents polish / web depth (Slice 6 largely closed) |
...
| Phase 5 — Collaboration / knowledge | In progress | ~78%     | Documents: `DOCUMENTS_VIEW_ACTIVITY` + `activityRevealed`; global `audit_logs` on `access_changed`; list-scope + FTS shipped |
...
2. Next normal slice: **Phase 5** — remaining Documents/Drive polish per plan; optional activity pagination; section `default_list_scope` admin UI.
