...
| 2026-04-29    | Mail & Notifications | Mail MVP (accounts, threads, outbound stub, cancel/reset, needs-link, sync-stub); inbox **`GET …/threads?q=`** subject search + web debounced field; in-app notify + **`in_app_notifications`** Prisma; Topbar feed → API (JWT). |
