...
- **Телефония / ATS (звонки):** Call core + Active Call Screen + CRM CALL activity + recording FileAsset/playback + click-to-call в runtime. Осталось по `08-Calls`: голый JSON webhook, history reconcile. DID / транскрипт / импорт Bitrix — later. — **L**
