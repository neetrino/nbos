...
- **Телефония / ATS (звонки):** Call core + incoming SSE popup + CRM CALL activity + recording FileAsset/playback + click-to-call в runtime. Осталось по `08-Calls`: голый JSON webhook, полный Call Screen, history reconcile. DID / транскрипт / импорт Bitrix — later. — **L**
