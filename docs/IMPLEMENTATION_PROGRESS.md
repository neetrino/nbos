...
| Current task         | **Bonus pool** (project-level) per NBOS; **Employee wallet** v1 shipped; **Payroll runs** + expense ledger; **Bonus board** API-backed; expense plan auto via **Scheduler** POST. |
| Status               | Open                                                                                                                                                                              |
...
- **Payroll / bonus:** **Bonus board** (`GET /api/bonus` + stats); **Payroll runs** + **APPROVED** → **`Expense`**; **`ExpensePayment`** ↔ salary lines + **`payroll_runs.total_paid`**; **Employee Wallet** read-only (`GET /api/me/wallet`, `/finance/wallet`, NBOS bonus pipeline groups). **Next:** project **bonus pool** aggregates per NBOS.
...
| Finance                     | Yes        | Yes          | Partial     | Partial | Orders/invoices/payments/subscriptions/expenses + reconciliation; partner filters; expense backlog + **Closed** + ledger, CSV, guards; **Expense Plan** + **manual + auto due Plan→Card** + **NBOS board** (`activeBoard`); **Bonus board** + **Payroll runs** + **My wallet** (`GET /api/me/wallet`); optional `expense_plan_id`; tab title helpers; legacy `/expenses` redirect |
