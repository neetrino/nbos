...
| Current task         | **Payroll** journal (expense cards from approved runs, partial pay); **Bonus board** API-backed; **Payroll runs** list/detail + workflow; expense plan auto via **Scheduler** POST. |
| Status               | Open                                                                                                                                                                                |
...
| Payroll / bonus pools / operational journal                                                                                              |             ~28% |
...
- **Payroll / bonus:** **Bonus board** (`GET /api/bonus` + stats); **Payroll runs** (`payroll_runs`, `salary_lines`, `GET/POST /api/payroll-runs`, status workflow, Finance **Payroll** tab + sidebar); expense cards from approved payroll not wired yet.
...
| Finance                     | Yes        | Yes          | Partial     | Partial | Orders/invoices/payments/subscriptions/expenses + reconciliation; partner filters; expense backlog + **Closed** + ledger, CSV, guards; **Expense Plan** + **manual + auto due Plan→Card** + **NBOS board** (`activeBoard`); **Bonus board** + **Payroll runs** (draft→closed, salary lines from HR base salary); optional `expense_plan_id`; tab title helpers; legacy `/expenses` redirect |
