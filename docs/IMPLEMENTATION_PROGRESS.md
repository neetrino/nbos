...
| Current task         | **Payroll** ledger sync (salary line paid ↔ expense payments); **Bonus board** API-backed; **Payroll runs** + **approved → expense cards**; expense plan auto via **Scheduler** POST. |
| Status               | Open                                                                                                                                                                                  |
...
| Payroll / bonus pools / operational journal                                                                                              |             ~38% |
...
- **Payroll / bonus:** **Bonus board** (`GET /api/bonus` + stats); **Payroll runs** + on **APPROVED** materialize **`Expense`** per payable line (`salary_lines.expense_id`, SALARY/BONUS category, `UNPAID`); web detail links to `/finance/expenses/:id`. **Next:** sync line `paid_amount` from expense ledger.
...
| Finance                     | Yes        | Yes          | Partial     | Partial | Orders/invoices/payments/subscriptions/expenses + reconciliation; partner filters; expense backlog + **Closed** + ledger, CSV, guards; **Expense Plan** + **manual + auto due Plan→Card** + **NBOS board** (`activeBoard`); **Bonus board** + **Payroll runs** (draft→closed; **APPROVED** → linked **`Expense`** per line); optional `expense_plan_id`; tab title helpers; legacy `/expenses` redirect |
