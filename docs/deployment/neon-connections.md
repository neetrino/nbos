...
| `DIRECT_URL`   | `pnpm db:migrate` / Prisma CLI only         | Neon **direct**             | `postgresql://neon_owner:***@ep-xxx.region.aws.neon.tech/neondb?sslmode=require`      |
...
2. Set `DIRECT_URL` only on migrate job / one-shot container.
