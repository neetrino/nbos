...
Дата: 2026-09-12. Статус: первый EN/RU срез в рабочем дереве. Этап 5 в работе: отчёт в coverage, 87 targeted tests зелёные. Не принято и не задеплоено. Live token-expiry, два пользователя, полный web lint и production rollout открыты.
...
| 4         | Частично                 | Desktop live: EN↔RU, light/dark/system на RU, Task/Meeting/Lead/Expense. Mobile 390px меню. Live Meeting: RU 409 overlap, network, 403 (перехват XHR, без записи встречи). Скрытая проба HY-шрифта в layout + `document.fonts.check` для Noto Sans Armenian. Targeted i18n/font/length tests зелёные. Не закрыты: live истечение access token (httpOnly session), logout/login двух пользователей, полный `web lint`. Auth-страницы вне scope |
| 5         | В работе                 | Отчёт в `I18N-COVERAGE.md` (Stage 5 report). 87 targeted tests зелёные. Не перенесено в IMPLEMENTATION_DONE. Открыты: два пользователя, live token-expiry, полный web lint, production rollout                                                                                                                                                                                                                                                |
