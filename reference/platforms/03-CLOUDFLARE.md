...
### NBOS Drive — папка Drive в R2.

Раздел **Drive** в системе использует один и тот же R2 bucket. Все файлы Drive сохраняются под префиксом **`Drive/`** в bucket:

- Структура ключей: `Drive/projects/{projectId}/{fileName}` (и подпапки).
- В R2 Dashboard объекты отображаются как папка **Drive** → **projects** → по проектам.
- Папку «Drive» создавать вручную не нужно: она появляется при первой загрузке файла через раздел Drive.

**Что сделать:**

1. Создать bucket в R2 (например, `nbos` или `nbos-drive`).
2. Создать API Token с Object Read & Write для этого bucket.
3. В `.env` (API) указать: `R2_ACCOUNT_ID`, `R2_BUCKET_NAME`, `R2_ACCESS_KEY_ID`, `R2_SECRET_ACCESS_KEY`, при необходимости `R2_PUBLIC_URL` для публичных ссылок.
4. В приложении открыть раздел **Drive**, выбрать проект и загрузить файлы — они появятся в R2 под префиксом `Drive/`.
