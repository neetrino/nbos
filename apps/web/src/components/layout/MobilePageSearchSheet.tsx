...
export function MobilePageSearchSheet({ search, label }: MobilePageSearchSheetProps) {
  ...
  return (
    <Sheet open={open} onOpenChange={setOpen}>
      ...
      >
        <SheetTitle className="sr-only">{resolvedLabel}</SheetTitle>
        <SheetDescription className="sr-only">{t('mobileDock.searchDescription')}</SheetDescription>
        <BottomSheetSwipeHandle handleRef={handleRef} />
        <div className="flex flex-col gap-3 px-4 pt-3 pb-5">
          <p className="text-foreground text-lg font-semibold tracking-tight">{resolvedLabel}</p>
