...
function ExpensesBacklogPageInner() {
  ...
  useEffect(() => {
    ...
    document.title = `Expense backlog${FINANCE_DOCUMENT_TITLE_SUFFIX}`;
    return () => {
  ...
  const replaceExpensesUrl = useCallback(
    (mutate: (params: URLSearchParams) => void) => {
      ...
      router.replace(q ? `${pathname}?${q}` : (pathname ?? EXPENSE_BACKLOG_LIST_PATH));
    },
...
export default function ExpensesBacklogPage() {
  return (
    <Suspense
      fallback={
