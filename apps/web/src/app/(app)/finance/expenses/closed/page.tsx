...
import { expenseClosedPageTitle } from '@/features/finance/constants/finance-route-page-titles';
import {
...
function ExpensesClosedPageInner() {
  const router = useRouter();
  const pathname = usePathname();
  const searchParams = useSearchParams();
  const projectIdFromUrl = searchParams.get(PROJECT_EXPENSES_DRILLDOWN_QUERY);

  useFinanceDocumentTitle(expenseClosedPageTitle(Boolean(projectIdFromUrl?.trim())));
  ...
  const replaceExpensesUrl = useCallback(
    (mutate: (params: URLSearchParams) => void) => {
      ...
      router.replace(q ? `${pathname}?${q}` : (pathname ?? EXPENSE_CLOSED_LIST_PATH));
    },
...
export default function ExpensesClosedPage() {
  return (
    <Suspense
      fallback={
