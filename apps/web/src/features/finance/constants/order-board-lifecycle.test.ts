...
describe('order-board-lifecycle', () => {
  it('active scope excludes terminal order statuses', () => {
    expect(matchesBoardLifecycleScope('PENDING_PAYMENT', ORDER_BOARD_STAGES, 'ACTIVE')).toBe(true);
    expect(matchesBoardLifecycleScope('ACTIVE', ORDER_BOARD_STAGES, 'ACTIVE')).toBe(true);
  ...
  it('closed scope includes only terminal outcomes', () => {
    ...
    expect(keys).toEqual(['FULLY_PAID', 'CLOSED']);
  });
});
