...
function useClientSessionSetters(
  ...
  const setOpenedConversation = useCallback((row: MessengerClientConversationRow | null) => {
  ...
  const setFilter = useCallback((value: 'all' | MessengerClientListFilter) => {
    setSnapshot((current) => ({ ...current, filter: value }));
  }, [setSnapshot]);
  const setProvider = useCallback((value: '' | MessengerClientProvider) => {
    setSnapshot((current) => ({ ...current, provider: value }));
  }, [setSnapshot]);
  const setUnlockedId = useCallback((id: string | null) => {
