...
export class NotificationEnqueueReconcileCron implements OnModuleInit, OnModuleDestroy {
  ...
  onModuleInit(): void {
    if (!shouldStartCronJob(NOTIFICATION_ENQUEUE_RECONCILE_CRON_ENABLED_ENV)) {
      this.logger.log(`Cron ${this.jobName} not registered (role/flags).`);
      return;
    }
    if (this.schedulerRegistry.doesExist('cron', this.jobName)) return;
    const expression =
      this.config.get<string>(NOTIFICATION_ENQUEUE_RECONCILE_CRON_ENV)?.trim() ||
  ...
  private async runSafely(): Promise<void> {
    ...
    try {
      await this.schedulerService.runNotificationEnqueueReconcile('cron');
    } catch (caught) {
      this.logger.error(`Notification enqueue reconcile cron failed`, caught);
    }
  }
}
