...
describe('resolvePaymentReminderRenderInput', () => {
  ...
  it('uses a coverage range and non-monthly kind for prepaid cards', () => {
    const resolved = resolvePaymentReminderRenderInput({
      code: 'INV-2026-0138',
      amount: 150000,
      taxStatus: 'TAX',
    ...
    expect(resolved?.renderInput.periodLabel.toLowerCase()).toContain('апрель');
    expect(resolved?.renderInput.periodLabel.toLowerCase()).toContain('июнь');
    expect(resolved?.renderInput.coverageMonthCount).toBe(3);
  });
});
