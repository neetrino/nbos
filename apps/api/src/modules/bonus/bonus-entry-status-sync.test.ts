...
describe('refreshBonusEntryStatusAfterReleasesChange', () => {
  ...
  it('promotes INCOMING to ACTIVE when counting releases exist', async () => {
    db.bonusEntry.findUnique.mockResolvedValue({ id: 'be1', status: 'INCOMING' });
    db.bonusRelease.aggregate.mockResolvedValue({ _sum: { amount: new Decimal(10) } });
  ...
  it('does not change PAID entries', async () => {
    db.bonusEntry.findUnique.mockResolvedValue({ id: 'be1', status: 'PAID' });
  ...
  it('demotes ACTIVE to INCOMING when no counting releases', async () => {
    db.bonusEntry.findUnique.mockResolvedValue({ id: 'be1', status: 'ACTIVE' });
    db.bonusRelease.aggregate.mockResolvedValue({ _sum: { amount: null } });
...
describe('syncBonusEntryStatusesForOrder', () => {
  it('refreshes each entry on the order', async () => {
    ...
    db.bonusEntry.findUnique
      .mockResolvedValueOnce({ id: 'a', status: 'INCOMING' })
