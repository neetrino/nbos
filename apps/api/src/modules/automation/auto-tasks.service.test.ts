...
describe('AutoTasksService', () => {
  ...
  describe('generateTasksForDeal', () => {
    ...
    it('should generate 5 tasks for DESIGN type', async () => {
      prisma.task.findFirst.mockResolvedValue(null);
      prisma.task.create.mockResolvedValue({ id: '1', code: 'T-2026-0001' });

      const result = await service.generateTasksForDeal('deal-2', 'DESIGN', 'user-1');
    ...
    it('should generate sequential codes', async () => {
      ...
      await service.generateTasksForDeal('deal-1', 'INTEGRATION', 'user-1');
